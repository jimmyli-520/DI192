...
function QuoteGenerator () {
  ...
  const pageStyle = {
    backgroundColor: color,         // The random color for the whole screen
    minHeight: "100vh",            // Ensures it covers the full height
    display: "flex",
    justifyContent: "center",      // Centers the quote box horizontally
    alignItems: "center",          // Centers the quote box vertically
    transition: "all 0.5s ease"
  };

  const cardStyle = {
    backgroundColor: "white",      // Keeps the text area readable
    padding: "40px",
    borderRadius: "15px",
  ...
  const brandStyle = {
    color: color,                  // The random color applied to text and buttons
    transition: "all 0.5s ease"
  };
  ...
  return (
    <div style={pageStyle}>
      <div style={cardStyle}>
        {/* Text uses the random color */}
        <h2 style={brandStyle}>"{quote.quote}"</h2>
        <p style={brandStyle}>- {quote.author}</p>
        
        {/* Button uses the random color for background, text stays white */}
        <button 
          onClick={getNewQuote} 
          style={{ 
